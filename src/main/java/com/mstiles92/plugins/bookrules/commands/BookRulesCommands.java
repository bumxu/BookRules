...
public class BookRulesCommands implements CommandHandler {
    ...
    @Command(name = "bookrules.debugfill", aliases = {"rulebook.debugfill", "rb.debugfill", "br.debugfill"}, permission = "bookrules.debugfill", playerOnly = true)
    public void debugfill(Arguments args) {
        int num = Integer.parseInt(args.getArgs()[0]);

        // clear out the current books
        for (StoredBook book : StoredBooks.getStoredBooks()) {
            StoredBooks.delete(book);
        }

        // Add numbered books for debug purposes
        for (int i = 0; i < num; i++) {
            ItemStack book = new ItemStack(Material.WRITTEN_BOOK);
            BookMeta meta = (BookMeta) book.getItemMeta();
            meta.setPages("Page 1", "Page 2", "Page 3");
            meta.setAuthor("Autogenerated");
            meta.setTitle("Book " + i);
            book.setItemMeta(meta);
            StoredBooks.add(book);
        }

        args.getPlayer().sendMessage(ChatColor.LIGHT_PURPLE + "Autogenerated " + num + " books!");
    }
